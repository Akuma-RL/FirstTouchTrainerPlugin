...
RT::Circle2D2::Circle2D2(Vector2 loc, float rad, int step, float lineThicc)
    : location(loc), radius(rad), steps(step), lineThickness(lineThicc) {}
...
void RT::Circle2D2::Draw(CanvasWrapper canvas) const
{
    ...
    for (int i = 0; i < steps; ++i)
    {
        int perc = static_cast<float>(i) / steps;
        int X = -sinf(CONST_PI_F * 2.f * perc);
        int Y = cosf(CONST_PI_F * 2.f * perc);
        ...
        circlePoints.emplace_back(Vector2{ X, Y });
    }
    ...
    for (size_t i = 0; i != circlePoints.size(); ++i)
    {
        Vector2 currentPoint = circlePoints[i];
        Vector2 nextPoint;
        if (i + 1 == circlePoints.size())
        {
            nextPoint = circlePoints[0];
