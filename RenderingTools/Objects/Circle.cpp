...
RT::Circle::Circle(Vector loc, Quat rot, float rad, int lineThicc, float pie, int step)
	: Circle() {
	location = loc; orientation = rot; radius = rad; lineThickness = lineThicc; piePercentage = pie; steps = step;
}
...
void RT::Circle::Draw(CanvasWrapper canvas, Frustum &frustum) const
{
	...
	float percentagePerLineSegment = 3.f / steps;
